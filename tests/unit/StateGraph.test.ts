...
import {
    StateGraphValidator
} from "../../src/state/StateGraphValidator";

import { StateGraphFactory } from "../../src/state/StateGraphFactory";
...
describe("StateGraph Models", () => {
    describe("StateGraphValidator", () => {
        ...
        test("should detect routes pointing to nonexistent states", () => {
            ...
            expect(errors.some((error) => error.includes("nonexistent"))).toBe(
                true
            );
        });
        ...
        test("should find unreachable states", () => {
            ...
            const unreachable = StateGraphValidator.findUnreachableStates(
                graph as any
            );
    ...
    describe("StateGraphFactory", () => {
        test("should create minimal StateGraph", () => {
            const graph = StateGraphFactory.createMinimal(
                "test-id",
                "Test Graph"
            );
        ...
        test("should create linear StateGraph", () => {
            ...
            const graph = StateGraphFactory.createLinear(
                "linear-test",
                "Linear Test",
                steps
            );
