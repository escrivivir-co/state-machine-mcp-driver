...
export abstract class BaseChannel<T extends BaseMessage> {
    ...
    constructor(
        ...
        if (enableLogging) {
            this.messages$
                .pipe(tap((message) => this.logMessage(message)))
                .subscribe();
        }
    }
    ...
    public send(message: Omit<T, "id" | "timestamp">): void {
        if (!this._isActive.value) {
            console.warn(
                `Channel ${this.channelName} is not active, message discarded`
            );
            return;
        }
        ...
        try {
            ...
            this.errorCount++;
            console.error(
                `Error sending message to ${this.channelName}:`,
                error
            );
    ...
    public subscribe(handler: (message: T) => void): () => void {
        const subscription = this.messages$.subscribe({
            ...
            error: (error) => {
                this.errorCount++;
                console.error(
                    `Error in ${this.channelName} subscription:`,
                    error
                );
            },
        });
    ...
    public filter<K extends T["type"]>(type: K): Observable<T & { type: K }> {
        return this.messages$.pipe(
            filter(
                (message): message is T & { type: K } => message.type === type
            )
        );
    }
    ...
    public mapToPayload<P = any>(): Observable<P> {
        return this.messages$.pipe(map((message) => message.payload as P));
    }
