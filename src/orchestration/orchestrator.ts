...
import { Subject, Observable, merge, combineLatest, timer, EMPTY } from 'rxjs';
import { 
  takeUntil, 
  tap, 
  filter, 
...
export class Orchestrator extends EventEmitter {
  ...
  constructor(config: OrchestratorConfig = {}) {
    ...
    this.config = {
      ...
      autoRegisterComponents: config.autoRegisterComponents ?? []
    };
  ...
  public async start(): Promise<void> {
    ...
    try {
      ...
      for (const component of this.config.autoRegisterComponents) {
        await this.registerComponent(component);
      }
      
      // Send startup notification
      this.sys.sendInfo('orchestrator', 'Orchestrator started successfully');
      ...
      this.sys.sendError('orchestrator', error as Error, 'Failed to start orchestrator');
      throw error;
    }
  }
  ...
  public async stop(): Promise<void> {
    ...
    try {
      ...
      this.sys.sendError('orchestrator', error as Error, 'Error during orchestrator shutdown');
      throw error;
    }
  }
  ...
  public destroy(): void {
    if (this.isRunning) {
      this.stop().catch(error => 
        console.error('Error during orchestrator stop in destroy:', error)
      );
    }
   ...
   */
  public async registerComponent(component: OrchestratorComponent): Promise<void> {
    ...
    try {
      ...
      if (this.config.enableLogging) {
        Logger.info(`📦 Component registered: ${component.name} (${component.id})`);
      }
  ...
  public async unregisterComponent(componentId: string): Promise<void> {
    ...
    try {
      ...
      if (this.config.enableLogging) {
        Logger.info(`📦 Component unregistered: ${component.name} (${componentId})`);
      }
  ...
  private setupChannelIntegration(): void {
    ...
    const allMessages$ = merge(
      this.app.messages$.pipe(map(msg => ({ channel: 'app', message: msg }))),
      this.sys.messages$.pipe(map(msg => ({ channel: 'sys', message: msg }))),
      this.ui.messages$.pipe(map(msg => ({ channel: 'ui', message: msg })))
    );
    ...
    allMessages$
      .pipe(
        ...
        catchError(error => {
          this.totalErrors++;
          this.sys.sendError('orchestrator', error as Error, 'Cross-channel routing error');
          return EMPTY;
        })
      )
   ...
   */
  private routeCrossChannelMessage(sourceChannel: string, message: any): void {
    const routeKey = `${sourceChannel}->${message.type}`;
    this.crossChannelRoutes.set(routeKey, (this.crossChannelRoutes.get(routeKey) || 0) + 1);
    ...
    this.app.messages$
      ...
      .subscribe(errorMessage => {
        this.emit('message:error', {
          ...
          error: new Error(JSON.stringify(errorMessage.payload) || 'Unknown app error')
        });
      });
    ...
    this.sys.messages$
      ...
      .subscribe(errorMessage => {
        this.emit('message:error', {
          channel: 'sys',
          messageId: errorMessage.id,
          error: new Error(errorMessage.payload?.message || 'Unknown sys error')
        });
      });
    ...
    this.ui.messages$
      ...
      .subscribe(errorMessage => {
        this.emit('message:error', {
          channel: 'ui',
          messageId: errorMessage.id,
          error: new Error(errorMessage.payload?.message || 'Unknown ui error')
        });
      });
  ...
  private setupStatistics(): void {
    ...
    timer(0, 30000) // Every 30 seconds
      ...
      .subscribe(() => {
        ...
        this.sys.sendInfo('orchestrator', 'Periodic statistics update', { stats });
      });
  }
  ...
  public getStatistics(): OrchestratorStatistics {
    ...
    return {
      ...
      errorRate: this.totalMessages > 0 ? this.totalErrors / this.totalMessages : 0,
   ...
   */
  public getMessagesByType$(type: string): Observable<{ channel: string; message: any }> {
   ...
   */
  public getMessagesBySource$(source: string): Observable<{ channel: string; message: any }> {
    return this.getAllMessages$().pipe(
