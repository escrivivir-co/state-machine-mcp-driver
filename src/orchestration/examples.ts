...
import { ChannelAgent, IOrchestratorChannels } from "./types";
...
export async function basicUsageExample(): Promise<void> {
    ...
    const subscription = orchestrator
        ...
        .subscribe(({ channel, message }) => {
            console.log(
                `📨 [${channel.toUpperCase()}] ${message.type} from ${
                    message.source
                }`
            );
        });
...
export async function advancedFilteringExample(): Promise<void> {
    ...
    orchestrator.app.stateTransitions$().subscribe((message) => {
        console.log(
            `🔄 State transition: ${message.payload.stateId} → ${message.payload.targetState}`
        );
    });
    ...
    orchestrator.ui.userInputs$().subscribe((message) => {
        console.log(
            `👤 User input from ${message.source}: ${
                message.payload.input || message.payload.command
            }`
        );
    });
    ...
    // Filter by source
    orchestrator
        .getMessagesBySource$("critical-service")
        .subscribe(({ channel, message }) => {
            console.log(
                `🚨 Critical service message on ${channel}: ${message.type}`
            );
        });
    ...
    orchestrator
        ...
        .subscribe(({ channel, message }) => {
            console.log(
                `⚠️ Error on ${channel} channel: ${JSON.stringify(
                    message.payload
                )}`
            );
        });
 ...
 */
class LoggerComponent implements ChannelAgent {
    ...
    async initialize(orchestrator: IOrchestratorChannels): Promise<void> {
        ...
        orchestrator.app.messages$.subscribe((message) =>
            this.logMessage("APP", message)
        );
        orchestrator.sys.messages$.subscribe((message) =>
            this.logMessage("SYS", message)
        );
        orchestrator.ui.messages$.subscribe((message) =>
            this.logMessage("UI", message)
        );
    ...
    async shutdown(): Promise<void> {
        if (this.channels) {
            this.channels.sys.sendInfo(
                this.id,
                `Custom Logger shutting down (logged ${this.logCount} messages)`
            );
        }
    }
    ...
    private logMessage(channel: string, message: any): void {
        this.logCount++;
        console.log(
            `📝 [${channel}] ${message.type}: ${JSON.stringify(
                message.payload
            )}`
        );
        ...
        if (this.logCount % 10 === 0) {
            this.channels?.sys.sendInfo(
                this.id,
                `Logged ${this.logCount} messages so far`
            );
        }
    }
}
...
export async function customComponentExample(): Promise<void> {
    ...
    for (let i = 0; i < 5; i++) {
        setTimeout(() => {
            orchestrator.app.sendActionRequest("demo", `action_${i}`, {
                index: i,
            });
            orchestrator.sys.sendInfo("demo", `Info message ${i}`);
            orchestrator.ui.sendDisplayUpdate(
                "demo",
                "display",
                "info",
                `Update ${i}`
...
export async function errorHandlingExample(): Promise<void> {
    ...
    orchestrator.on("orchestrator:started", (data) => {
        console.log(
            `🚀 Orchestrator started at ${new Date(
                data.timestamp
            ).toISOString()}`
        );
    });
    ...
    orchestrator.on("component:registered", (data) => {
        console.log(
            `📦 Component registered: ${data.componentName} (${data.componentId})`
        );
    });

    orchestrator.on("message:error", (data) => {
        console.log(
            `❌ Message error on ${data.channel}: ${data.error.message}`
        );
    });
...
export async function performanceExample(): Promise<void> {
    ...
    for (let i = 0; i < messageCount; i++) {
        ...
        if (channel === 0) {
            orchestrator.app.sendActionRequest(
                `source-${i % 10}`,
                `action-${i}`,
                { data: i }
            );
    ...
    console.log(
        `📤 Sent ${messageCount} messages in ${sendTime}ms (${Math.round(
            (messageCount / sendTime) * 1000
        )} msg/s)`
    );
    ...
    setTimeout(() => {
        ...
        console.log("📊 Performance Statistics:", {
            ...
            messagesPerSecond: Math.round(
                stats.totalMessages / (stats.uptime / 1000)
            ),
            channels: {
