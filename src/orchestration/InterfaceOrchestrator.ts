...
export class InterfaceOrchestrator extends EventEmitter {
  ...
  connectChatProvider(chatProvider: OllamaChatProvider): void {
    ...
    const mcpClient = this.mcpAdapter.getNativeDriver();
  ...
  async enableAgentControl(): Promise<void> {
    ...
    for (const server of servers) {
      try {
        ...
        Logger.warn(`Could not load agent prompt for ${server.id}`, { error });
      }
    }
  ...
  private async processEventQueue(): Promise<void> {
    ...
    while (this.eventQueue.length > 0) {
      ...
      try {
        ...
        Logger.error('Failed to process interface event', error as Error, event);
  ...
  private async processInterfaceEvent(event: InterfaceEvent): Promise<void> {
    Logger.debug(`Processing ${event.source} event: ${event.type}`, event.data);
   ...
   * Register MCP tools with chat provider
   */
  private async registerToolsWithChatProvider(mcpClient: MCPClientDriver): Promise<void> {
        ...
        for (const tool of tools) {
          this.chatProvider.registerTool({
            ...
            execute: async (params: any) => {
              return await mcpClient.executeTool(server.id, tool.name, params);
            }
          });
        }
        
        Logger.info(`Registered ${tools.length} tools from ${server.id} with chat provider`);
      } catch (error) {
        Logger.warn(`Could not register tools from ${server.id}`, { error });
      }
    }
  }
  ...
  private setupMCPToUIBridge(mcpClient: MCPClientDriver): void {
    ...
    mcpClient.on('server:health-changed', (data) => {
      this.ui!.displayHealthStatus({
        ...
        message: `Server ${data.serverId} is ${data.healthy ? 'online' : 'offline'}`
      });
    });
    ...
    if (this.chatProvider) {
      ...
      if (conversation) {
        this.chatProvider.send(
          conversation.id,
          `Action executed: ${action.type} - ${result.success ? 'Success' : 'Failed'}`
        );
      }
    }
