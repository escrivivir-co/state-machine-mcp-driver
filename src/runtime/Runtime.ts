...
export class Runtime extends EventEmitter {
  ...
  async initialize(): Promise<void> {
    try {
      ...
      const errorMessage = `Failed to initialize runtime: ${error instanceof Error ? error.message : 'Unknown error'}`;
    ...
    try {
      ...
      if (agent.mcpServerId) {
        try {
          ...
          Logger.warn(`Could not load prompt for agent ${agent.id}`, { error });
        }
      }
  ...
  getCurrentStateNode(): any {
    ...
    if (!stateNode) {
      throw new Error(`State node '${this.currentState.currentStateId}' not found`);
    }
   ...
   */
  async transitionTo(stateId: string, trigger = 'manual', metadata?: Record<string, any>): Promise<void> {
    ...
    try {
      ...
      const errorMessage = `Transition failed: ${error instanceof Error ? error.message : 'Unknown error'}`;
      Logger.error(errorMessage, error as Error, {
        from: fromState,
        to: stateId,
        trigger
  ...
  async executeAction(action: AgentAction): Promise<AgentActionResult> {
    ...
    try {
      ...
      if (!AgentUtils.canExecuteAction(agent, action.type)) {
        throw new Error(`Agent ${action.agentId} cannot execute action ${action.type}`);
      }
  ...
  private async validateTransition(fromState: string, toState: string): Promise<boolean> {
    const currentStateNode = this.getCurrentStateNode();
    const validRoute = currentStateNode.routes?.find((route: any) => route.target === toState);
  ...
  private async executeStateActions(actions: string[], phase: 'enter' | 'exit'): Promise<void> {
    for (const action of actions) {
      try {
        ...
        Logger.warn(`Error executing ${phase} action: ${action}`, { error });
      }
    }
  }
  ...
  private updateTransitionStats(executionTime: number): void {
    ...
    this.stats.performance.averageTransitionTime = 
      (this.stats.performance.averageTransitionTime * (this.stats.transitionsCount - 1) + executionTime) / 
      this.stats.transitionsCount;
  }
  ...
  private updateActionStats(executionTime: number): void {
    ...
    this.stats.performance.averageActionTime = 
      (this.stats.performance.averageActionTime * (this.stats.actionsExecuted - 1) + executionTime) / 
      this.stats.actionsExecuted;
  }
}
