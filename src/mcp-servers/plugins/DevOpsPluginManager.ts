...
import { IDevOpsPlugin, PluginConfig, PluginContext, PluginResult } from './IDevOpsPlugin.js';
...
export class DevOpsPluginManager {
  ...
  async initializePlugin(pluginId: string): Promise<void> {
    ...
    try {
      ...
      entry.active = true;
      Logger.mcpInfo(`Plugin ${entry.config.name} initialized successfully`);
      
    } catch (error) {
      Logger.mcpError(`Failed to initialize plugin ${pluginId}`, { error });
      throw error;
    }
  }
  ...
  async executePluginCommand(
    ...
    try {
      ...
      Logger.mcpError(`Plugin ${pluginId} command ${command} failed`, { error });
      return {
        success: false,
  ...
  async setPluginEnabled(pluginId: string, enabled: boolean): Promise<void> {
    ...
    Logger.mcpInfo(`Plugin ${pluginId} ${enabled ? 'enabled' : 'disabled'}`);
  }
