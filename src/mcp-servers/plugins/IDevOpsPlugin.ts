...
export abstract class BaseDevOpsPlugin implements IDevOpsPlugin {
  ...
  protected requireContext(): PluginContext {
    if (!this.context) {
      throw new Error(`Plugin ${this.config.name} not initialized - context not available`);
    }
    return this.context;
  }
  ...
  protected requireMCPAdapter(): MCPDriverAdapter {
    ...
    if (!context.mcpAdapter) {
      throw new Error(`Plugin ${this.config.name} requires MCP adapter but none is available`);
    }
