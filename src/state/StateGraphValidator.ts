...
import { Route, State, StateGraph, StateNode, StateTransition } from "@/models";
import { ValidationOptions, ValidationResult, Validators } from "@/utils";

export class StateGraphValidator {
    /**
     * Validates a StateGraph object for structural integrity
     */
    static validateBasicStateGraph(graph: StateGraph): string[] {
        const errors: string[] = [];

        // Check if initial state exists
        if (!graph.states[graph.initialState]) {
            errors.push(
                `Initial state '${graph.initialState}' not found in states`
            );
        }
        ...
        Object.values(graph.states).forEach((state) => {
            ...
            state.routes.forEach((route) => {
                if (!graph.states[route.target]) {
                    errors.push(
                        `Route '${route.id}' in state '${state.id}' points to non-existent state '${route.target}'`
                    );
                }
            });
            ...
            const duplicateIds = routeIds.filter(
                (id, index) => routeIds.indexOf(id) !== index
            );
            if (duplicateIds.length > 0) {
                errors.push(
                    `Duplicate route IDs in state '${
                        state.id
                    }': ${duplicateIds.join(", ")}`
                );
            }
     ...
     */
    static findUnreachableStatesV1(graph: StateGraph): string[] {
        ...
        return Object.keys(graph.states).filter(
            (stateId) => !reachable.has(stateId)
        );
    }
     ...
     */
    static validateIds(state: State): string[] {
        const errors: string[] = [];
    ...
    static validate(
        ...
        const missingProps = Validators.hasRequiredProperties(
            graph,
            requiredProps
        );
        ...
        if (!Validators.isValidId(graph.id)) {
            errors.push(
                "StateGraph ID must contain only alphanumeric characters, dashes, and underscores"
            );
        }
    ...
    static validateStateNode(
        ...
        if (!Validators.isValidId(state.id)) {
            errors.push(
                "State ID must contain only alphanumeric characters, dashes, and underscores"
            );
        }
        ...
        if (graph && state.id !== graph.initialState) {
            const hasIncomingRoutes = Object.values(graph.states).some(
                (otherState) =>
                    otherState.routes.some((route) => route.target === state.id)
            );
            if (!hasIncomingRoutes) {
                warnings.push(
                    `State '${state.id}' has no incoming routes and may be unreachable`
                );
            }
        }
    ...
    static validateRoute(route: Route, graph?: StateGraph): ValidationResult {
        ...
        if (!Validators.isValidId(route.id)) {
            errors.push(
                "Route ID must contain only alphanumeric characters, dashes, and underscores"
            );
        }
        ...
        if (route.condition && !Validators.isNonEmptyString(route.condition)) {
            errors.push(
                "Route condition must be a non-empty string if provided"
            );
        }
	...
	static validateState(state: State): ValidationResult {
		...
		if (!Validators.isValidId(state.graphId)) {
			errors.push(
				"Graph ID must contain only alphanumeric characters, dashes, and underscores"
			);
		}
		...
		if (!Validators.isValidId(state.currentStateId)) {
			errors.push(
				"Current state ID must contain only alphanumeric characters, dashes, and underscores"
			);
		}
	...
	static validateStateTransition(
		...
		if (!Validators.isValidId(transition.fromState)) {
			errors.push(
				"From state ID must contain only alphanumeric characters, dashes, and underscores"
			);
		}

		if (!Validators.isValidId(transition.toState)) {
			errors.push(
				"To state ID must contain only alphanumeric characters, dashes, and underscores"
			);
		}
