...
import { GamificationUI, BaseGamificationUIConfig } from '../ui/GamificationUI';
...
export const X_PLUS_1_MULTI_UI: MultiUIGameConfig = {
  game: {
    ...
    version: '1.0.0',
    description: 'The X+1 incremental decision game with multiple UI interfaces'
  },
  ...
  ui: [
    {
      ...
      config: {
        ...
        welcomeMessage: 'Welcome to X+1 Game on Console! Type your decisions and watch the magic happen.',
        debugMode: false,
    ...
    },
    {
      ...
      config: {
        ...
        welcomeMessage: 'Welcome to X+1 Game in your browser! Click, tap, and interact.',
        debugMode: false,
        maxMessagesPerThread: 50,
        enablePostulations: true,
        port: 8080,
...
export function validateMultiUIConfig(config: MultiUIGameConfig): string[] {
  ...
  const duplicateIds = uiIds.filter((id, index) => uiIds.indexOf(id) !== index);
  if (duplicateIds.length > 0) {
    errors.push(`Duplicate UI IDs found: ${duplicateIds.join(', ')}`);
  }
  
  // Check for duplicate ports
  const webUIs = config.ui.filter(ui => ui.type === 'html5' && ui.enabled && ui.config.port);
  ...
  if (config.orchestration?.primaryUIId) {
    const primaryUI = config.ui.find(ui => ui.id === config.orchestration.primaryUIId);
    if (!primaryUI) {
      errors.push(`Primary UI '${config.orchestration.primaryUIId}' not found`);
    } else if (!primaryUI.enabled) {
      errors.push(`Primary UI '${config.orchestration.primaryUIId}' is not enabled`);
    }
  }
