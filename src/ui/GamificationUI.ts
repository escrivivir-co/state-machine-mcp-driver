...
export abstract class GamificationUI extends EventEmitter {
     ...
     */
    abstract displayAgentPostulations(
    ...
    private setupRxJSStreams(): void {
        ...
        const allStreams$ = merge(
            ...
            this.userInput$.pipe(
                map((input) => ({ type: "userInput", data: input }))
            ),
            this.agentSelection$.pipe(
                map((selection) => ({
                    type: "agentSelection",
                    data: selection,
                }))
            ...
            this.mcpEvents$.pipe(
                map((event) => ({ type: "mcpEvent", data: event }))
            ),
    ...
    private setupMCPIntegration(): void {
        ...
        if (driver && typeof driver.on === "function") {
            Object.values(MCPEventType).forEach((eventType) => {
                driver.on(eventType, (event: MCPEvent) => {
    ...
    public generateAgentPostulations(
        ...
        this.emit(GamificationUIEvent.POSTULATIONS_GENERATED, {
            postulations,
            context: fullContext,
        });
     ...
     */
    public async requestAgentSelection(
        postulations?: AgentPostulation[]
    ): Promise<void> {
        if (!this.config.enablePostulations) {
            return;
        ...
        this.emit(GamificationUIEvent.AGENT_SELECTION_REQUESTED, {
    ...
    public selectAgent(index: number): boolean {
        ...
        this.emit(GamificationUIEvent.AGENT_SELECTED, {
    ...
    protected completeCurrentThread(): void {
        if (this.currentThread) {
            ...
            this.currentThread$.next(this.currentThread);
            this.emit(GamificationUIEvent.THREAD_COMPLETED, {
                thread: this.currentThread,
            });
        }
    }
    ...
    private async processAgentSelection(
        selection: AgentPostulation
    ): Promise<void> {
        this.awaitingAgentSelection = false;
        this.pendingPostulations = [];
        ...
        switch (event.type) {
            ...
            default:
                if (this.config.debugMode) {
                    Logger.mcpVerbose("GamificationUI: Unhandled MCP event", {
                        event,
                    });
                }
        }
    ...
    private logEvent(event: { type: string; data: any }): void {
        if (this.config.debugMode) {
            Logger.mcpVerbose("GamificationUI Event", {
                type: event.type,
                data: event.data,
            });
        }
