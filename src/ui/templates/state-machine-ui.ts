...
} from "@/chat-provider/mcpTools";
import { MCPDriverAdapter, MCPServerConfig } from "@/drivers";
...
import {
  GAME_CONFIG,
  MESSAGE_TEMPLATES,
} from "@examples/xplus1-app/xplus1-game";
...
export class StateMachineUI extends ConsoleGamificationUI {
  ...
  private constructor(
    ...
    if (!runtime || !runtime.initialize) {
      console.warn("Recreating the base StateMachineUI!!!");
      const runtimeConfig = getBasicRuntimeConfig({} as AppConfig);
  ...
  private async syncWithMCPState(): Promise<void> {
    if (!this.mcpDriver?.executeTool || !this.mcpSyncEnabled) return;
  ...
  async create(
    ...
    console.log("StateMachineUI Starting the creations process!!!!!!");
    const config = {
      ...DEFAULT_APP_CONFIG,
      mcp: {
        servers: {
          "devops-mcp-server": {} as BaseMCPServerConfig,
          "state-machine-server": {} as BaseMCPServerConfig,
          "wiki-mcp-browser": {} as BaseMCPServerConfig,
        },
      },
    };
    console.log("StateMachineUI Adding MCP DEFINITIONS");
    Object.keys(config.mcp.servers).forEach(async (serverKey) => {
      console.log("addServer", serverKey);
      let server = getConfigOrDefault(serverKey, config);
      let transportConfig = parseMcpConfigToTransportConfig(server);
      console.log("addServer with config", serverKey, transportConfig);
  ...
  async start(): Promise<void> {
    ...
    console.warn("Warning, reseting StateMachineUI!!");
    await this.create(this.runtime, this.mcpDriver, this.chatProvider);
    ...
    console.log(
      MESSAGE_TEMPLATES.turnStart(this.gameState.x, this.gameState.messageCount)
    );
    console.log(
  ...
  private async generateAgentMessage(
    ...
    try {
      ...
      return (
        enhancedMessage || this.generateFallbackMessage(agentId, postulation)
      );
    } catch (error) {
  ...
  private generateFallbackMessage(
    ...
    const agentTemplates =
      templates[agentId as keyof typeof templates] || templates["justice-bot"];
  ...
  protected async showStatus(): Promise<void> {
    ...
    console.log(
      `  Active agents: ${
        ((await this.runtime?.getAgents()) || []).length || 0
      }`
    );
    console.log(
      `  User simulator: ${
