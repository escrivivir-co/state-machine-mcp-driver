...
import { GamificationUI, GamificationUIEvent, GameMessage, GameThread, BaseGamificationUIConfig } from '../ui/GamificationUI';
...
class ConsoleGamificationUIWrapper extends GamificationUI {
  ...
  constructor(runtime: Runtime, mcpAdapter: MCPDriverAdapter, config: ConsoleUIConfig) {
  ...
  async displayAgentPostulations(postulations: any[]): Promise<void> {
    // Delegate to console UI
    if (typeof (this.consoleUI as any).displayAgentPostulations === 'function') {
      await (this.consoleUI as any).displayAgentPostulations(postulations);
    }
  }
...
export class MultiUIGameManager extends EventEmitter {
  ...
  private allUIEvents$ = new Subject<{ uiId: string; event: string; data: any }>();
  ...
  async start(): Promise<void> {
    ...
    Logger.info(`Starting Multi-UI Game Manager for '${this.config.game.name}'`);
    ...
    try {
      ...
      Logger.info(`Multi-UI Game Manager started with ${this.uiInstances.size} UI instances`);
      
    } catch (error) {
      Logger.error('Failed to start Multi-UI Game Manager', error as Error);
      await this.stop();
      throw error;
    }
  }
  ...
  async stop(): Promise<void> {
    ...
    const stopPromises = Array.from(this.uiInstances.values()).map(async (instance) => {
      try {
        ...
        Logger.error(`Error stopping UI ${instance.config.id}`, error as Error);
      }
    });
   ...
   */
  async broadcastMessage(message: GameMessage, excludeUIId?: string): Promise<void> {
    const broadcastPromises = Array.from(this.uiInstances.values())
      .filter(instance => instance.config.id !== excludeUIId && instance.isStarted)
      .map(async (instance) => {
        try {
          await instance.ui.displayMessage(message);
        } catch (error) {
          Logger.error(`Error broadcasting to UI ${instance.config.id}`, error as Error);
          instance.errorCount++;
        }
      });
  ...
  getStats(): {
    ...
    const activeUIs = Array.from(this.uiInstances.values()).filter(i => i.isStarted);
    const uptime = this.primaryUI?.startTime ? Date.now() - this.primaryUI.startTime : undefined;
  ...
  private async createUIInstances(): Promise<void> {
    const enabledUIs = this.config.ui.filter(uiConfig => uiConfig.enabled);
    
    for (const uiConfig of enabledUIs) {
      try {
        Logger.info(`Creating UI instance: ${uiConfig.name} (${uiConfig.type})`);
        
        const ui = UIFactory.create(uiConfig.type, uiConfig, this.runtime, this.mcpAdapter);
        ...
        // Set primary UI
        if (uiConfig.config.isPrimary || uiConfig.id === this.config.orchestration?.primaryUIId) {
        ...
        Logger.error(`Failed to create UI instance ${uiConfig.id}`, error as Error);
        throw error;
      }
    }
    ...
    if (!this.primaryUI && this.uiInstances.size > 0) {
      ...
      Logger.info(`Using ${this.primaryUI.config.id} as primary UI (auto-selected)`);
    }
  }

  private async startUIInstances(): Promise<void> {
    // Sort by priority (higher priority first)
    const sortedInstances = Array.from(this.uiInstances.values())
      .sort((a, b) => (b.config.config.priority || 0) - (a.config.config.priority || 0));
    ...
    for (const instance of sortedInstances) {
      try {
        ...
        Logger.error(`Failed to start UI instance ${instance.config.id}`, error as Error);
        instance.errorCount++;
    ...
    for (const instance of this.uiInstances.values()) {
      if (instance.isStarted) {
        try {
          ...
          if (typeof (instance.ui as any).handleGameStateUpdate === 'function') {
            await (instance.ui as any).handleGameStateUpdate(state);
          }
          instance.lastSync = Date.now();
        } catch (error) {
          Logger.error(`Error syncing state to UI ${instance.config.id}`, error as Error);
          instance.errorCount++;
        }
      }
    }
    ...
    for (const instance of this.uiInstances.values()) {
      if (instance.isStarted) {
        try {
          ...
          if (typeof (instance.ui as any).handleThreadUpdate === 'function') {
            await (instance.ui as any).handleThreadUpdate(thread);
          }
        } catch (error) {
          Logger.error(`Error syncing thread to UI ${instance.config.id}`, error as Error);
          instance.errorCount++;
        }
      }
    }
  ...
  private logUIEvent(event: { uiId: string; event: string; data: any }): void {
