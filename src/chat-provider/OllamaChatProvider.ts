    ...
    private async registerMCPTools(): Promise<void> {
        ...
        for (const server of servers) {
            try {
                ...
                console.log(
                    `Found ${tools.length} tools from server ${server.id}`
                );
            } catch (error) {
                console.warn(
                    `Could not register tools from ${server.id}`,
                    error
                );
            }
        }
        ...
        try {
            ...
            switch (name) {
                ...
                case "mcp_call_tool": {
                    ...
                    if (!toolName)
                        throw new Error("mcp_call_tool: missing toolName");
                    if (!serverId)
                        throw new Error("mcp_call_tool: no server available");
                    return await this.mcpDriver.executeTool(
                        serverId,
                        toolName,
                        params
                    );
                ...
                case "mcp_get_prompt": {
                    ...
                    const variables: Record<string, any> | undefined =
                        args.variables;
                    const serverId = args.serverId || servers[0]?.id;
    ...
    private async callOllama(
        ...
        while (attempt <= this.cfg.maxRetries) {
            try {
                ...
                console.log(
                    `📊 Response done: ${data.done}, model: ${data.model}`
                );
                ...
                if (
                    ...
                    console.log(
                        `⚠️ Empty response detected, checking thinking field...`
                    );
