...
function parseToolCallFromText(text: string): ToolCall | null {
  ...
  for (const obj of candidates) {
    ...
    if (obj.tool && obj.tool.name) {
      const args = typeof obj.tool.arguments === 'string' ? obj.tool.arguments : JSON.stringify(obj.tool.arguments ?? {});
      return {
        id: uuidv4(),
        type: 'function',
    ...
    if (Array.isArray(obj.tool_calls) && obj.tool_calls.length > 0) {
      ...
      return {
        id: call.id ?? uuidv4(),
        type: 'function',
        function: { name: call.function?.name ?? 'unknown', arguments: args },
      };
    }
  }
  return null;
...
export class OllamaChatProvider extends EventEmitter {
  ...
  startConversation(userId?: string, tools?: Tool[], metadata?: Record<string, any>): ConversationContext {
    const id = uuidv4();
    const now = Date.now();
  ...
  endConversation(conversationId: string): boolean {
    ...
    if (existed) this.stats.activeConversations = Math.max(0, this.stats.activeConversations - 1);
    return existed;
  }
  ...
  const instr = buildToolInstruction(ctx.mcpTools);
    ...
    this.stats.averageResponseTime =
      (this.stats.averageResponseTime * (totalResponses - 1) + elapsed) / totalResponses;
    this.stats.conversationsByModel[req.model] = (this.stats.conversationsByModel[req.model] ?? 0) + 1;

    // Add assistant message
    const assistantMsg: ChatMessage = { role: 'assistant', content: resp.message.content };
    ...
    if (this.cfg.enableMCP && this.mcpDriver) {
      ...
      if (toolCall) {
        ...
        const { valid, errors } = validateArgs(declared?.function.parameters, parsedArgs);
        if (!valid) {
          ...
          const errorPayload = {
            ...
            schema: {
              ...
              properties: Object.fromEntries(
                Object.entries(declared?.function.parameters.properties ?? {}).map(([k, v]: [string, any]) => [k, { type: (v as any)?.type }])
              ),
            },
            received: parsedArgs,
          };
          ...
          if (options?.toolAutoInvoke) {
            ...
            const followAssistant: ChatMessage = { role: 'assistant', content: followResp.message.content };
            ctx.messages.push(followAssistant);
          }
        } else {
          // Execute tool (support provider-handled MCP meta-tools)
          const toolResult = await this.executeMCPInvocation(toolCall.function.name, parsedArgs);
  ...
  private async executeMCPInvocation(name: string, args: Record<string, any>): Promise<any> {
  ...
  private async callOllama(req: ChatCompletionRequest): Promise<OllamaChatResponse> {
    ...
    throw lastErr instanceof Error ? lastErr : new Error('Ollama chat request failed');
  }

  private toChatCompletionResponse(model: string, message: ChatMessage): ChatCompletionResponse {
    return {
      id: uuidv4(),
