...
export const config: AppConfig = {
  ...
  logging: {
    ...
    enableFile: parseEnvBoolean(process.env.LOG_ENABLE_FILE, process.env.NODE_ENV === 'production'),
  ...
  session: {
    ...
    cleanupInterval: parseEnvNumber(process.env.SESSION_CLEANUP_INTERVAL, 300000) // 5 minutes
  },
  ...
  performance: {
    maxConcurrentRequests: parseEnvNumber(process.env.MAX_CONCURRENT_REQUESTS, 100),
  ...
  development: {
    hotReload: parseEnvBoolean(process.env.HOT_RELOAD, process.env.NODE_ENV === 'development'),
    debug: parseEnvBoolean(process.env.DEBUG, process.env.NODE_ENV === 'development'),
    mockServers: parseEnvBoolean(process.env.MOCK_SERVERS, process.env.NODE_ENV === 'development')
  }
};
...
export class ConfigManager {
  ...
  static validate(config: AppConfig): string[] {
    ...
    config.mcpServers.forEach((server, index) => {
      ...
      if (!server.name) errors.push(`MCP server ${index}: Name is required`);
      if (!server.url) errors.push(`MCP server ${index}: URL is required`);
   ...
   */
  static forEnvironment(env: 'development' | 'production' | 'test'): Partial<AppConfig> {
    switch (env) {
      case 'development':
        return {
          logging: {
