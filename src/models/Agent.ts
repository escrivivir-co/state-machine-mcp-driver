...
export class AgentFactory {
  ...
  static createAgent(config: AgentConfig): Agent {
    return {
      ...
      name: config.name,
      role: config.role,
      status: config.autoStart ? AgentStatus.ACTIVE : AgentStatus.INACTIVE,
   ...
   */
  static createNarrator(id: string, name: string, mcpServerId?: string): AgentConfig {
    return {
      id,
      name,
      role: AgentRole.NARRATOR,
      description: 'Narrative agent that provides story context and descriptions',
      mcpServerId,
      autoStart: true,
      priority: 100,
      config: {
   ...
   */
  static createGuide(id: string, name: string, mcpServerId?: string): AgentConfig {
    return {
      id,
      name,
      role: AgentRole.GUIDE,
      description: 'Guide agent that helps users navigate and understand the system',
      mcpServerId,
      autoStart: true,
      priority: 80,
      config: {
...
export class AgentUtils {
  ...
  static canExecuteAction(agent: Agent, actionType: string): boolean {
    ...
    switch (agent.role) {
      ...
      case AgentRole.GUIDE:
        return ['guide', 'help', 'suggest', 'explain'].includes(actionType);
   ...
   */
  static updateStats(agent: Agent, type: 'action' | 'message' | 'error'): void {
   ...
   */
  static calculatePriorityScore(agent: Agent, context?: Record<string, any>): number {
