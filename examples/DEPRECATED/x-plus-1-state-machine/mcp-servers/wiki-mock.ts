...
import MCPWikiBrowserServer from "@/mcp-servers/MCPWikiBrowserServerImpl";
import {
    loadWikiTopics,
    loadWikiContent,
    loadWikiMessages,
...
export class MockWikiClient {
    ...
    async browseTopic(
        topic: string,
        agentType: "dionisio" | "apolo"
    ): Promise<string> {
        this.currentTopic = topic;

        const topics =
            agentType === "dionisio" ? DIONISIO_TOPICS : APOLO_TOPICS;
    ...
    private generateCosmicContent(topic: string): string {
        ...
        return (
            cosmicContent[topic as keyof typeof cosmicContent] ||
    ...
    private generateHistoricalContent(topic: string): string {
        ...
        return (
            historicalContent[topic as keyof typeof historicalContent] ||
