...
class DualModeGameManager {
  ...
  async start(): Promise<void> {
    try {
      ...
      this.runtime = new Runtime(this.runtimeConfig.mcpServerId, this.runtimeConfig.graphId);
      ...
      Logger.info('DualModeGameManager: All interfaces started successfully');
  ...
  private async setupMCPServers(): Promise<void> {
    ...
    Logger.info('MCP servers health check', { results: Array.from(healthResults.entries()) });
  }
  ...
  private async setupConsoleUI(): Promise<void> {
    this.consoleUI = new ConsoleGamificationUI(
      this.runtime,
      {
        ...
        enableColors: true,
        welcomeMessage: 'Welcome to the X+1 Game! Console interface is active.'
      }
    );
  ...
  private async setupWebUI(): Promise<void> {
    this.webUI = new HTML5GamificationUI(
      this.runtime,
      this.mcpAdapter,
      {
  ...
  private setupWebEventHandlers(): void {
    ...
    this.webUI.on('agentSelected', (data) => {
      if (this.consoleUI) {
        console.log(`[Web] Agent selected: ${data.postulation.agent.name} - ${data.postulation.reason}`);
      }
    });
  }
   ...
   */
  public async broadcastMessage(content: string, type: 'system' | 'info' = 'system'): Promise<void> {
    ...
    if (this.webUI) {
      promises.push(this.webUI.displayNotification('Broadcast', content, 'info'));
    }
