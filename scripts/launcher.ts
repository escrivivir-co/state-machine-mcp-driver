...
export class ApplicationLauncher {
    ...
    private async launchMultiUI(config: MultiUIGameConfig): Promise<void> {
        ...
        if (errors.length > 0) {
            throw new Error(
                `Invalid Multi-UI configuration:\n${errors.join("\n")}`
            );
        }
        ...
        return new Promise((resolve, reject) => {
            ...
            pullProcess.on("close", (code) => {
                ...
                if (code === 0) {
                    ...
                    reject(
                        new Error(
                            `Failed to pull model ${modelName}. Exit code: ${code}`
                        )
                    );
                }
            });
    ...
    private async startMCPServiceLauncher(): Promise<void> {
        ...
        launcherProcess.on("close", (code) => {
            if (!this.isShuttingDown) {
                console.error(
                    `❌ MCP Service Launcher exited with code ${code}`
                );
            }
        ...
        if (!ready) {
            throw new Error(
                "MCP Service Launcher did not become ready in time"
            );
        }
        console.log(
    ...
    private async launchMCPServers(): Promise<void> {
        ...
        if (!this.mcpDriver) {
            throw new Error(
                "MCP Driver not initialized. Service launcher must be started first."
            );
        }
        ...
        try {
            ...
            if (Array.isArray(result) && result[0]?.type === "text") {
                try {
                    ...
                    if (process.env.MCP_QUIET !== "true") {
                        console.warn(
                            "⚠️ Failed to parse launch results:",
                            error
                        );
                    }
    ...
    private async performHealthChecks(): Promise<void> {
        ...
        if (!this.mcpDriver) {
            console.warn(
                "⚠️ MCP Driver not available, performing basic health checks"
            );
        ...
        try {
            ...
            // Handle wrapped text response format
            if (
                Array.isArray(healthResults) &&
                healthResults[0]?.type === "text"
            ) {
                try {
                    ...
                    if (process.env.MCP_QUIET !== "true") {
                        console.warn(
                            "⚠️ Failed to parse health check results:",
                            error
                        );
                    }
            ...
            if (parsedResults.success && parsedResults.healthCheck) {
                ...
                for (const [serverId, result] of Object.entries(results)) {
                    ...
                    if (healthResult.status === "healthy") {
                        console.log(
                            `✅ ${healthResult.name || serverId}: Healthy`
                        );
                    } else {
                        console.log(
                            `❌ ${healthResult.name || serverId}: ${
                                healthResult.status
                            } - ${healthResult.error || "Unknown issue"}`
                        );
                        allHealthy = false;
                    }
                }
                ...
                console.warn(
                    "⚠️ Health check failed or returned unexpected results"
                );
            ...
            console.error(
                "❌ Failed to perform health checks via service launcher:",
                error
            );
            console.log("Falling back to basic health checks...");
            await this.performBasicHealthChecks();
    ...
    private loadMultiUIConfig(target: string): MultiUIGameConfig | null {
        ...
        try {
            ...
            if (errors.length > 0) {
                console.warn(
                    `⚠️ Invalid Multi-UI configuration: ${errors.join(", ")}`
                );
                return null;
            }
            ...
            console.warn(
                `⚠️ Could not load Multi-UI config from ${configPath}:`,
     ...
     */
    private async waitForMCPHealthy(
        ...
        for (let attempt = 1; attempt <= maxAttempts; attempt++) {
            ...
            for (const serverId of serverIds) {
                try {
                    ...
                    if (process.env.MCP_QUIET !== "true") {
                        console.log(
                            `Health check attempt ${attempt}/${maxAttempts} failed for ${serverId}:`,
                            error
                        );
                    }
                    break;
            ...
            if (allHealthy) {
                console.log(
                    `✅ All MCP servers healthy (attempt ${attempt}/${maxAttempts})`
                );
                return;
            }
            ...
            (answer: string) => {
                ...
                killProcess.on("close", (code: number) => {
                    if (code === 0) {
                        ...
                        console.log(
                            "ℹ️  No Node.js processes were found to terminate"
                        );
                    } else {
                        console.log(
                            `⚠️  Process termination completed with exit code: ${code}`
                        );
                    }
                    resolve();
                });
...
function showHelp(): void {
    ...
    console.log("Targets:");
    console.log(
        "  x-plus-1    Launch the X+1 State Machine game example (default)"
    );
    console.log(
        "  custom      Launch a custom script (requires customScript path)"
    );
    console.log("");
    console.log("Options:");
    console.log(
        "  --kill-all-node    Terminate all Node.js processes on the system"
    );
